function drawAllCharts(){
  ...
  if(mhist.length>=2) makeChart("chart-margin",{grid:{left:56,right:20,top:26,bottom:30},tooltip:TT,
    ...
    series:[{name:"两融合计",type:"line",smooth:true,symbol:"circle",symbolSize:9,showSymbol:true,
      ...
      emphasis:{itemStyle:{color:BLUE,borderColor:BLUE}},areaStyle:{color:BLUE,opacity:.08}}]});
  ...
  // 恐贪×沪深300 双轴两线（2026-09-16 修复：原仅恐贪单线，邱总09-04起要求两线叠加）
  makeChart("chart-fg",{grid:{left:44,right:46,top:16,bottom:30},
    legend:{data:["恐贪","沪深300"],bottom:0,textStyle:{fontSize:10},itemWidth:14,itemHeight:8},
    xAxis:{type:"category",data:fh.map(x=>x.d),show:false,boundaryGap:false},
    yAxis:[{type:"value",name:"恐贪",min:0,max:100,...AXIS_STYLE},{type:"value",name:"HS300",scale:true,...AXIS_STYLE,splitLine:{show:false}}],tooltip:TT,
    series:[
      {name:"恐贪",type:"line",data:fh.map(x=>x.fear),smooth:true,symbol:"circle",symbolSize:4,lineStyle:{width:2,color:GOLD},itemStyle:{color:GOLD},
        areaStyle:{color:GOLD,opacity:.12},markLine:{silent:true,data:[{yAxis:25},{yAxis:45}],lineStyle:{color:"#d8c28f",type:"dashed"},label:{fontSize:9}}},
      {name:"沪深300",type:"line",yAxisIndex:1,data:fh.map(x=>x.hs),smooth:true,symbol:"none",lineStyle:{color:BLUE,width:2},itemStyle:{color:BLUE}}]});
  // 情绪周期曲线
